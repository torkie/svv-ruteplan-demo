...
import { IFeature } from "../Model/CameraResponse";
...
const crs = new L.Proj.CRS("EPSG:25833", "+proj=utm +zone=33 +ellps=GRS80 +units=m +no_defs", {
...
export class RuteplanMap extends React.Component<IRuteplanMapProps, IRuteplanMapState> {
  ...
  cameraPointToLayer = (point: Feature<any, any>, latlng: L.LatLng): L.Layer => {
  ...
  render() {
    ...
    return <div><Map zoomControl={false} center={position} zoom={4} id="map" crs={crs} contextmenu={true} contextmenuItems={contextMenuItems}>
      ...
      <Modal
        isOpen={this.state.modalCameraIsOpen}
        onRequestClose={this.closeCameraModal}
        style={customCameraStyles}
        contentLabel="Example Modal">
            <div className="cameraContent">
            <h2> Vegkamera</h2>
            <div> Kamera Id: { this.state.openedCamera!=null && this.state.openedCamera.properties.CAMERA_ID}  </div>
            <div> Sted: { this.state.openedCamera!=null && this.state.openedCamera.properties.ORIENTATION_DESCRIPTION}  </div>
            <div> Vegnummer : { this.state.openedCamera!=null && this.state.openedCamera.properties.ROAD_NUMBER}  </div>
            <div> Beskrivelse: { this.state.openedCamera!=null && this.state.openedCamera.properties.DESCRIPTION}  </div>
            <div> Vær : { this.state.openedCamera!=null && <a href= { this.state.openedCamera.properties.STILL_IMAGE_URL_DESCRIPTION } target="_blank" > {this.state.openedCamera.properties.STILL_IMAGE_URL_DESCRIPTION}</a>}  </div>     
